...
func httpError(w io.WriteCloser, ctx *ProxyCtx, err error) {
	if _, err := io.WriteString(w, "HTTP/1.1 502 Bad Gateway\r\n\r\n"); err != nil {
		log.Errorf("[Session: %v] Fail to respond to client, the error is: %v", ctx.Sess, err)
	}
	if err := w.Close(); err != nil {
		log.Errorf("[Session: %v] Fail to close the client connection, the error is: %v", ctx.Sess, err)
	}
}
...
func copyAndHalfClose(proxyctx *ProxyCtx, dst, src *net.TCPConn) {
	if _, err := io.Copy(dst, src); err != nil {
		log.Errorf("[Session: %v] Error copying to clients, the error is: %v", proxyctx.Sess, err)
	}
